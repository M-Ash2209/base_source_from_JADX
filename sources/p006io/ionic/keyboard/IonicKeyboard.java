    ...
    public boolean execute(String str, JSONArray jSONArray, final CallbackContext callbackContext) throws JSONException {
        if ("hide".equals(str)) {
            this.f59cordova.getThreadPool().execute(new Runnable() {
                public void run() {
                    InputMethodManager inputMethodManager = (InputMethodManager) IonicKeyboard.this.f59cordova.getActivity().getSystemService("input_method");
                    View currentFocus = IonicKeyboard.this.f59cordova.getActivity().getCurrentFocus();
            ...
            this.f59cordova.getThreadPool().execute(new Runnable() {
                public void run() {
                    ((InputMethodManager) IonicKeyboard.this.f59cordova.getActivity().getSystemService("input_method")).toggleSoftInput(0, 1);
                    callbackContext.success();
                }
            });
            return true;
        } else if (!PushConstants.INITIALIZE.equals(str)) {
            return false;
        } else {
            this.f59cordova.getThreadPool().execute(new Runnable() {
                public void run() {
                    DisplayMetrics displayMetrics = new DisplayMetrics();
                    IonicKeyboard.this.f59cordova.getActivity().getWindowManager().getDefaultDisplay().getMetrics(displayMetrics);
                    final float f = displayMetrics.density;
                    IonicKeyboard ionicKeyboard = IonicKeyboard.this;
                    View unused = ionicKeyboard.rootView = ionicKeyboard.f59cordova.getActivity().getWindow().getDecorView().findViewById(16908290).getRootView();
                    ViewTreeObserver.OnGlobalLayoutListener unused2 = IonicKeyboard.this.list = new ViewTreeObserver.OnGlobalLayoutListener() {
                        ...
                        public void onGlobalLayout() {
                            ...
                            if (Build.VERSION.SDK_INT >= 21) {
                                Display defaultDisplay = IonicKeyboard.this.f59cordova.getActivity().getWindowManager().getDefaultDisplay();
