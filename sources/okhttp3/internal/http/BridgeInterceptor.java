...
public final class BridgeInterceptor implements Interceptor {
    ...
    public Response intercept(Interceptor.Chain chain) throws IOException {
        ...
        if (request.header(HttpRequest.HEADER_USER_AGENT) == null) {
            newBuilder.header(HttpRequest.HEADER_USER_AGENT, Version.userAgent());
        }
        ...
        if (z && HttpRequest.ENCODING_GZIP.equalsIgnoreCase(proceed.header(HttpRequest.HEADER_CONTENT_ENCODING)) && HttpHeaders.hasBody(proceed)) {
            ...
            request2.body(new RealResponseBody(proceed.header(HttpRequest.HEADER_CONTENT_TYPE), -1, Okio.buffer((Source) gzipSource)));
        }
