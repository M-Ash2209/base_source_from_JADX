...
public class CordovaBridge {
    ...
    private boolean verifySecret(String str, int i) throws IllegalAccessException {
        if (!this.jsMessageQueue.isBridgeEnabled()) {
            if (i == -1) {
                LOG.m37d(LOG_TAG, str + " call made before bridge was enabled.");
                return false;
            }
            LOG.m37d(LOG_TAG, "Ignoring " + str + " from previous page load.");
            return false;
        } else if (this.expectedBridgeSecret >= 0 && i == this.expectedBridgeSecret) {
            return true;
        } else {
            LOG.m40e(LOG_TAG, "Bridge access attempt with wrong secret token, possibly from malicious code. Disabling exec() bridge!");
            clearBridgeSecret();
    ...
    public String promptOnJsPrompt(String str, String str2, String str3) {
        if (str3 != null && str3.length() > 3 && str3.startsWith("gap:")) {
            ...
            LOG.m40e(LOG_TAG, "gap_init called from restricted origin: " + str);
            return "";
        }
    }
}
