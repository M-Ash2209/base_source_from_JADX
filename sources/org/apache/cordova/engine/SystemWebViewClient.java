...
public class SystemWebViewClient extends WebViewClient {
    ...
    public void onReceivedError(WebView webView, int i, String str, String str2) {
        if (this.isCurrentlyLoading) {
            LOG.m39d(TAG, "CordovaWebViewClient.onReceivedError: Error code=%s Description=%s URL=%s", Integer.valueOf(i), str, str2);
            if (i == -10) {
                this.parentEngine.client.clearLoadTimeoutTimer();
    ...
    public void onReceivedSslError(WebView webView, SslErrorHandler sslErrorHandler, SslError sslError) {
        try {
            if ((this.parentEngine.f64cordova.getActivity().getPackageManager().getApplicationInfo(this.parentEngine.f64cordova.getActivity().getPackageName(), 128).flags & 2) != 0) {
    ...
    public WebResourceResponse shouldInterceptRequest(WebView webView, String str) {
        try {
            if (!this.parentEngine.pluginManager.shouldAllowRequest(str)) {
                LOG.m49w(TAG, "URL blocked by whitelist: " + str);
            ...
            if (!(e instanceof FileNotFoundException)) {
                LOG.m41e(TAG, "Error occurred while loading a file (returning a 404).", (Throwable) e);
            }
            return new WebResourceResponse("text/plain", HttpRequest.CHARSET_UTF8, (InputStream) null);
        }
    }
