...
/* renamed from: de.appplant.cordova.plugin.notification.trigger.MatchTrigger */
public class MatchTrigger extends IntervalTrigger {
    ...
    private static DateTrigger.Unit getUnit(List<Integer> list, List<Integer> list2) {
        ...
        DateTrigger.Unit unit2 = INTERVALS[list.indexOf((Object) null) + 1];
        if (list2.get(0) != null) {
    ...
    private Date getTriggerDate(Date date) {
        ...
        if (baseTriggerDate.get(2) >= cal.get(2)) {
            if (baseTriggerDate.get(6) >= cal.get(6)) {
                if (baseTriggerDate.get(11) >= cal.get(11)) {
    ...
    private boolean setDayOfWeek(Calendar calendar) {
        ...
        if (this.matchers.get(4) == null || calendar.get(1) == i3) {
            return true;
        }
        return false;
    }
